...
class MainApiController extends Controller
{
    ...
    {
        $shipment=Shipment::whereNotNull('ordered_at')->get();
        return $this->generateResponse(ShipmentResource::collection($shipment));
    }
    public function usersList()
    {
        $users=User::all();
        return $this->generateResponse($users);
    }

    public function editShipmentsAccessResponse(Request $request)
    {
        $shipment=Shipment::find($request->get('shipment_id'));
        if($shipment->accessResponse == 'denied'){
            $shipment->accessResponse='granted';
        }else{
            $shipment->accessResponse = 'denied';
        }
        $shipment->save();
        $shipment->refresh();
        return $this->generateResponse($shipment);
