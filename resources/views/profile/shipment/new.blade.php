@component('admin.layouts.content' , ['title' => 'سفارش جدید'])
    ...
    <script>
        ...
        var map2 = L.map('map2').setView([32.48842419646451, 54.17070052948507], 4);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://osm.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(map2);
        var marker2 = '';
        map2.on('click', function (ev) {
            var latlng = map2.mouseEventToLatLng(ev.originalEvent);
            ...
            marker2 = L.marker([latlng.lat, latlng.lng]).addTo(map2)
            console.log(latlng.lat + ', ' + latlng.lng);

            originLongAddress.value = latlng.lng;
            originLatAddress.value = latlng.lat;
        });


        var map = L.map('map').setView([32.48842419646451, 54.17070052948507], 4);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://osm.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(map);
        var marker = '';
        map.on('click', function (ev) {
            ...
            destinationLongAddress.value = latlng.lng;
            destinationLatAddress.value = latlng.lat;
        });
